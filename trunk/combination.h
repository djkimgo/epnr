...
BidIt r_begin, BidIt r_end)
{
  ...
  for(BidIt r_it1=tmp_r_end; r_it1!=r_begin || r_it1==r_begin; r_it1 -= 3, n_it1 -= 3)
  {
    if( (*r_it1 == *n_it1) && (*(r_it1+1) == *(n_it1+1)) && (*(r_it1+2) == *(n_it1+2)) )
    {
      if(r_it1!=r_begin) //to ensure not at the start of r sequence
      {
        boolmarked=true;
        r_marked=(r_it1-3);
        r_it1 += 3;//add it back again 
        continue;
      }
    ...
    else //if(*r_it1!=*n_it1 )
    {
      ...
      if(boolmarked==true)
      {
        ...
        for (BidIt n_it2=n_begin;n_it2!=n_end;n_it2 += 3)
          if ( (*r_marked==*n_it2) && (*(r_marked+1)==*(n_it2+1)) && (*(r_marked+2)==*(n_it2+2)) ) {n_marked=n_it2;break;}
      
		n_marked += 3;
        BidIt n_it3=n_marked;
		
        for  (BidIt r_it2=r_marked;r_it2!=r_end;r_it2 += 3,n_it3 += 3)
        {
          *r_it2=*n_it3;
		  *(r_it2+1)=*(n_it3+1);
		  *(r_it2+2)=*(n_it3+2);
      ...
      for(BidIt n_it4=n_begin; n_it4!=n_end; n_it4 += 3)
        if( (*r_it1==*n_it4) && (*(r_it1+1)==*(n_it4+1)) && (*(r_it1+2)==*(n_it4+2)) )
        {
			n_it4 += 3;
          *r_it1=*(n_it4);
		  *(r_it1+1)=*(n_it4+1);
